...
## Sprint 12: 마무리 — 보안 강화 / 성능 / 학생부 커스터마이징 / 사용자 관리

### 배경

남은 Jira 이슈 7개(`SD2-5, 11, 20, 23, 25, 26, 28`)를 모두 수용 기준 기준으로 완료 처리한다.
이전 스프린트에서 부분 구현된 이슈(SD2-20/23/25/26)는 갭만 채우고, 미착수 이슈(SD2-11/28)는 신규 구현한다.

**설계 결정 — 학생부 항목 커스터마이징(SD2-11) 스키마 전략**

| 대안 | 탈락 이유 |
|------|-----------|
| `AcademicRecord`에 컬럼 직접 추가 | 항목이 유동적이라 스키마 마이그레이션이 반복 발생 |
| 별도 `FieldDefinition` 컬렉션 + `AcademicRecord.custom_fields: Map` | 항목 메타데이터(label/visibility) 분리, 기존 데이터 호환 무관 (선택) |
| JSON Schema 기반 동적 검증 | 복잡도 과잉, 교육용 프로젝트 규모에 부적합 |

→ `FieldDefinition` 컬렉션으로 항목 정의를 관리하고 `AcademicRecord.custom_fields: Map<fieldId, value>` 방식 채택. 항목 삭제 시 기존 데이터는 orphan으로 보존(호환 정책).

**설계 결정 — 사용자 관리(SD2-28) API 범위**

| 대안 | 탈락 이유 |
|------|-----------|
| 기존 demo-users 시스템 유지 + 시딩 UI | 역할 변경이 즉시 반영 안 됨, 수용 기준 미충족 |
| 완전한 Auth 서비스 분리(별도 마이크로서비스) | 스코프 과잉 |
| 단일 `/api/admin/users` CRUD + JWT 재발급 유도 | 현 아키텍처 최소 확장, 역할 변경 즉시 반영 가능 (선택) |

→ 교사 RBAC 아래 `/api/admin/users` CRUD를 추가하고, 역할 변경 시 해당 사용자 세션 무효화(쿠키 삭제 + AuditLog 기록) 방식으로 즉시 반영 보장.

---

### 태스크

#### 12-A: SD2-20 완료 — 보고서 파일 메타정보 보강
**수용 기준 갭**: 파일에 생성시각 + 대상 범위가 명시되어야 한다.

**PDF (`ReportsPage.tsx`)**
- 현재: `생성일: {new Date().toLocaleDateString("ko-KR")}` (날짜만, 시각 없음)
- 변경: `생성일시: YYYY-MM-DD HH:mm` + `대상 학기: {최초 학기} ~ {최신 학기}` 텍스트 추가

**Excel (`handleExcelDownload`)**
- 현재: 메타정보 시트 없음
- 변경: 첫 번째 시트 "메타정보"에 `{ 항목, 값 }` 형식으로 학생명/생성일시/대상 학기 범위/다운로드 권한 역할 포함
- `XLSX.utils.book_append_sheet` 순서: 메타정보 → 성적 상세 → 학기별 요약

**E2E 시나리오 추가** (`e2e/grades.spec.ts` 또는 `reports.spec.ts`)
- 교사 로그인 → 보고서 탭 → PDF 버튼 클릭 확인 (다운로드 트리거 검증)

---

#### 12-B: SD2-23 완료 — HTTPS 443 리스너 + 민감정보 마스킹
**수용 기준 갭**: 전송 구간 보안(HTTPS) 설정이 Terraform에 반영되지 않았다.

**Terraform (`terraform/main.tf`)**
- ALB에 443 리스너 추가: `aws_alb_listener.https`
  - `protocol = "HTTPS"`, `ssl_policy = "ELBSecurityPolicy-TLS13-1-2-2021-06"`
  - `certificate_arn`: `aws_acm_certificate_validation.teacher_erp.certificate_arn`
  - `default_action`: 클라이언트 타겟 그룹으로 포워드
- 80→443 리다이렉트 리스너 변경: `aws_alb_listener.http`의 `type = "redirect"`

**API 응답 민감정보 마스킹 확인 및 보강**
- `GET /api/students/*` 응답에서 User 문서의 `password` 필드가 반환되지 않는지 확인
- `demoUsers` / DB User 조회 결과에서 `password` 제거 (이미 없으면 문서화로 대체)
- 로그(`accessLog`, `writeSecurityEvent`)에서 Authorization 헤더 값이 로깅되지 않는지 확인

**수용 기준 체크리스트** (코드/문서로 명시)
- [x] 저장 데이터 보호 — Atlas M0 encryption at rest 기본 적용 (`docs/infra/07-backup-restore-runbook.md` 참조)
- [x] 전송 구간 보안 — HTTPS 443 리스너 추가로 완료
- [x] 최소 권한 접근 — CASL ability 기반 전 API 적용
- [x] 보안 이벤트 감사로그 — `SecurityEvent` 스키마 + `GET /api/security/events`

---

#### 12-C: SD2-25 완료 — 누락 인덱스 + 성능 SLO 문서화
**수용 기준 갭**: feedback/counseling 스키마에 `student_id` 인덱스가 없어 학생별 조회 시 컬렉션 스캔 발생.

**`packages/shared/db/src/schemas/feedback.ts`**
- 인덱스 추가:
  ```
...
**`packages/shared/db/src/schemas/counseling.ts`**
- 인덱스 추가:
  ```
...
**`docs/infra/08-performance-slo.md`** 신규 문서
- 목표 응답 시간: 목록 API < 200ms, 보고서 생성 < 800ms, OLAP 스냅샷 조회 < 300ms
- 동시성: 최대 20 concurrent users 기준으로 ECS 1태스크로 처리 (초과 시 ALB + Auto Scaling 고려)
- 모니터링 지표: ECS CPU/메모리, ALB TargetResponseTime, MongoDB Atlas Metrics
- 측정 방법: `accessLog`의 `duration_ms` 필드 기반 CloudWatch Logs Insights 쿼리 예시 포함

---

#### 12-D: SD2-26 완료 — 복구 정합성 검증 스크립트
**수용 기준 갭**: "복구 테스트를 통해 데이터 정합성이 검증된다"가 코드/스크립트로 존재해야 한다.

**`scripts/verify-integrity.ts`** 신규
- MongoDB 연결 후 아래 검증 수행:
  1. 학생-학부모 연결 무결성: `parent_student` 컬렉션의 `student_id` 모두 User에 존재하는지
  2. 성적 계산 일관성: `grade.calculated_grade !== calcGrade(grade.score)` 건수 집계
  3. OLAP 스냅샷 일치: `StudentLearningSnapshot`의 `avg_score`가 운영 성적과 10% 이상 차이나는 학생 목록
  4. 감사 로그 연속성: 최근 24시간 C/U/D AuditLog 건수 0이면 경고
- 검증 결과를 stdout에 JSON으로 출력, 오류 건수 > 0이면 exit code 1
- `docs/infra/07-backup-restore-runbook.md`에 `npx tsx scripts/verify-integrity.ts` 실행 지시 추가

---

#### 12-E: SD2-28 구현 — 사용자 관리 API + UI

**서버: `packages/app/server/src/routes/admin.ts`** 신규
- 모든 엔드포인트 TEACHER 전용, `authorize` 미들웨어로 보호

| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | `/api/admin/users` | 전체 사용자 목록 (role 필터 지원) |
| POST | `/api/admin/users` | 신규 사용자 생성 (role 필수) |
| PATCH | `/api/admin/users/:id` | 역할·이름·담당학생 수정 |
| DELETE | `/api/admin/users/:id` | 사용자 삭제 (본인 삭제 불가) |
| POST | `/api/admin/users/:id/parent-links` | 학부모-자녀 연결 |
| DELETE | `/api/admin/users/:id/parent-links/:studentId` | 연결 해제 |

- 역할(role) 변경 시: `writeAuditLog` 호출 (collection: `users`, operation: `UPDATE`, before/after 포함)
- 사용자 삭제 시: 관련 ParentStudent 레코드도 cascade 삭제

**클라이언트: `packages/app/client/src/pages/AdminPage.tsx`** 신규
- 경로: `/admin` (TEACHER만 접근)
- 탭: "사용자 관리" / "학부모-자녀 연결"
- 사용자 목록 테이블: 이름, 역할, 학년/반/번호(학생), 이메일, 액션(수정/삭제)
- 신규 사용자 다이얼로그: 역할 선택 → 역할별 필수 입력 폼
- 역할 변경 셀렉트: 변경 후 "변경 저장" 확인 모달
- `AppShell.tsx` 교사 네비게이션에 "관리자" 링크 추가

**`shared/types`에 AdminUser 타입 추가**
- `CreateUserInput`, `UpdateUserInput`, `AdminUserListResponse`

**E2E: `e2e/admin.spec.ts`**
- 교사 로그인 → `/admin` 진입 → 신규 학생 계정 생성 확인
- 학생 계정으로 `/admin` 접근 → 403 확인

---

#### 12-F: SD2-11 구현 — 학생부 항목 커스터마이징

**`packages/shared/db/src/schemas/field-definition.ts`** 신규
```ts
// FieldDefinition: 학교가 정의하는 학생부 커스텀 항목 메타데이터
// 설계: 항목 삭제 시 기존 AcademicRecord.custom_fields 데이터는 orphan으로 보존 (호환 정책)
{
  field_id: String (unique, slug형태 예: "extracurricular_activity"),
  label: String,          // 화면 표시명
  field_type: Enum("text" | "number" | "date"),
  visibility: Enum("TEACHER_ONLY" | "PUBLIC"), // PUBLIC = 학생/학부모도 조회 가능
  is_active: Boolean,     // false면 신규 편집 불가, 기존 데이터 조회만 가능
...
**`AcademicRecord` 스키마 확장**
- `custom_fields: { type: Map, of: String, default: () => new Map() }` 추가

**서버: `packages/app/server/src/routes/field-definitions.ts`** 신규

| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | `/api/field-definitions` | 전체 항목 정의 목록 (TEACHER: 전부, STUDENT/PARENT: visibility=PUBLIC만) |
| POST | `/api/field-definitions` | 항목 생성 (TEACHER only) |
| PATCH | `/api/field-definitions/:fieldId` | label/visibility/is_active 수정 (TEACHER only) |

**`students.ts` 확장**
- `PUT /:id/academic-record` 핸들러에서 `custom_fields` 업데이트 처리
- `GET /:id/academic-record` 응답 시: 요청자가 STUDENT/PARENT이면 `visibility=TEACHER_ONLY` 항목을 custom_fields에서 제거

**클라이언트: 학생 상세 페이지 학생부 탭 확장**
- `StudentDetailPage.tsx`의 학생부(AcademicRecord) 탭에 커스텀 필드 렌더링 추가
- TEACHER: 커스텀 필드 편집 가능 + "항목 관리" 링크 (→ `/admin#field-definitions`)
- STUDENT/PARENT: `visibility=PUBLIC`인 항목만 표시

**`AdminPage.tsx`에 "항목 관리" 탭 추가**
- 커스텀 항목 목록 (field_id, label, visibility, is_active)
- 신규 항목 생성 폼
- 항목 비활성화 토글 (삭제 대신 `is_active = false`)

**E2E: `e2e/field-definitions.spec.ts`**
- 교사 로그인 → 항목 관리 → 커스텀 항목 생성 → 학생 상세에서 커스텀 필드 입력 확인
- 학생 계정으로 `visibility=TEACHER_ONLY` 항목이 조회 안 되는지 확인

---

#### 12-G: SD2-5 Epic 완료 처리
SD2-11 구현 완료 후 수용 기준 체크:
- [x] 학생 기본 정보·출결·특기사항 등록/수정/조회 (`PUT /:id/academic-record`)
- [x] 항목별 권한 정책 (TEACHER_ONLY vs PUBLIC, CASL으로 편집 제한)
- [x] 변경 사항 이력 추적 (`AuditLog`, `/students/:id/audit`)
- [x] 학생/학부모 조회 시 허용 정보만 노출 (visibility 필터)

→ SD2-11 완료 확인 후 SD2-5를 Done으로 전환.

---

### 완료 순서 (우선순위)

1. **12-C** (인덱스 추가) — 코드 변경 최소, DB 성능 즉시 개선
2. **12-D** (검증 스크립트) — 단일 파일, 빠른 완료
3. **12-A** (보고서 메타정보) — UI 소폭 수정
4. **12-B** (HTTPS Terraform) — 인프라 변경, `terraform plan` 확인 후 적용
5. **12-E** (사용자 관리) — 중간 규모 신규 기능
6. **12-F** (학생부 커스터마이징) — 가장 복잡, 신규 스키마 포함
7. **12-G** (Epic 완료) — 12-F 완료 후 Jira 상태 전환
