# 학생 정보 및 성적 관리 시스템 (Teacher ERP) 요구사항 명세서
...
- 운영 DB(트랜잭션 컬렉션)와 분석 DB(집계 컬렉션)를 동일 MongoDB 인스턴스 내 별도 컬렉션으로 구분하여 관리한다.
- 운영 컬렉션(성적·피드백·상담·출결)의 변경 이벤트를 MongoDB Change Streams로 감지하고, 분석 컬렉션을 즉시 갱신한다. (이벤트 기반 파이프라인)
- 이벤트 기반 갱신이 실패하거나 누락된 경우를 대비해 주기적 배치 재집계(cron)를 병행 운영한다.

#### 2. 분석 집계 스키마

- **`StudentLearningSnapshot` (학생별 학습 현황 스냅샷)**: 학생·학기 단위로 전체 평균 점수, 종합 등급, 과목별 점수 배열, 출결 요약, 피드백 수, 상담 수를 저장한다.
- **`SubjectProgressSummary` (과목별 진도 요약)**: 학생·과목 단위로 학기별 점수 이력, 해당 과목 평균, 추세(상승/하락/유지)를 저장한다.
- 분석 컬렉션은 읽기 전용으로 취급하며, 오직 파이프라인 워커만 쓰기 권한을 가진다.

#### 3. 분석 대시보드

- 교사는 학생별·과목별 학습 현황을 집계 데이터 기반 대시보드에서 조회할 수 있어야 한다.
- 대시보드는 운영 컬렉션에 직접 쿼리하지 않고 분석 컬렉션을 읽는다.
- 레이더 차트(과목별 점수), 선 차트(기간별 추세), 카드형 요약(평균·등급·상담 횟수)을 제공한다.

#### 4. AI 챗봇 (선택)

- 분석 컬렉션에 집계된 학생별 학습 요약 데이터를 컨텍스트로 삼아 Claude API를 호출하는 AI 챗봇을 구현한다.
- 교사는 자연어로 학생 학습 현황을 질의하고, AI가 집계 데이터를 바탕으로 응답한다.
- 학생 개인정보 보호: 챗봇 컨텍스트에 개인 식별 정보(이름·학번)를 포함할 경우 교사 권한 인증 후에만 접근 가능하다.
...
- 교사는 보고서를 PDF 나 Excel 파일 형식으로 다운로드할 수 있어야 한다.
...
- **상담 내역 관리**: 중간
- **알림 기능**: 중간
- **OLAP 파이프라인 (Change Streams + 배치)**: 중간
- **분석 대시보드**: 중간
- **보고서 생성**: 낮음
- **AI 챗봇**: 낮음 (선택)
