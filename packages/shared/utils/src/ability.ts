...
import { IUser, IParentUser } from '@teacher-erp/shared-types';

export type Actions = 'manage' | 'create' | 'read' | 'update' | 'delete';
export type Subjects = 'User' | 'Student' | 'Teacher' | 'Parent' | 'Grade' | 'Feedback' | 'Counseling' | 'all';
...
export function defineAbilityFor(user: IUser): AppAbility {
  ...
  if (user.role === 'TEACHER') {
    ...
    can('read', 'Feedback', { studentId: { $in: parentUser.children }, isPublicToParent: true });
  }

  return build({
    detectSubjectType: (object: any) => object.__t as ExtractSubjectType<Subjects> || 'all'
  });
}
